...
# Carga los datos
ds <- readRDS("data/datasets/HS_FinalDS_Jun2024_FS.rds")
...
# After merging baseline and endline
ds <- merge(x = ds, y = ds_endline, by.x = "start_p10", by.y = "start_p10_endline")

# 1. Normalize suffixes (.x → _baseline, .y → _endline)
names(ds) <- sub("\\.x$", "_baseline", names(ds))
names(ds) <- sub("\\.y$", "_endline", names(ds))

# 2. Fix double suffixes that appear as _endline_endline or _endline_baseline
names(ds) <- sub("_endline_endline$", "_endline", names(ds))
names(ds) <- sub("_endline_baseline$", "_baseline", names(ds))

# 3. Remove or fix duplicated column names if any
if (anyDuplicated(names(ds)) > 0) {
  dupes <- names(ds)[duplicated(names(ds))]
  message("Duplicate column names found: ", paste(unique(dupes), collapse = ", "))
  ds <- ds[, !duplicated(names(ds))]  # keep first occurrence, drop rest
}


ds <- ds %>% mutate(
  cambio_vitalidad = i_vitalidad_endline - i_vitalidad_baseline,
  cambio_rol_emocional = i_rol_emocional_endline - i_rol_emocional_baseline,
  cambio_salud_mental = i_salud_mental_endline - i_salud_mental_baseline,
  cambio_animo = i_animo_endline - i_animo_baseline,
  cambio_satisfaccion = satisfaccion_endline - satisfaccion_baseline,
  cambio_extra = i_extra_endline - i_extra_baseline,
  cambio_mental = i_mental_endline - i_mental_baseline
)
...
saveRDS(base_matching,"data/datasets/base_matching_FS.rds")
write.csv(base_matching, "data/datasets/base_matching_FS.csv", row.names = FALSE)