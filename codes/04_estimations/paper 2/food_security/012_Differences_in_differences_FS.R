...
# -------------------------
# FS: construir outcomes panel
# -------------------------
# 1) Traer FSI baseline (desde el corte transversal, que tiene FSI y FSI_theta)
base_src <- ds_matched %>%
...
# =========================
# DiD con FE de hogar: Omitir el término principal de Tratamiento
# =========================
# Nota: con FE de hogar, el término 'tratamiento_control' es colineal y debe excluirse.
# El parámetro de interés es la interacción Tratamiento×Post.
library(estimatr)

rFS <- lm(FSI_panel ~ tratamiento_control + time + tratamiento_control:time + factor(start_p10),data=panel,weights=weights)

rFS_theta <- lm(FSI_theta_panel ~ tratamiento_control + time + tratamiento_control:time + factor(start_p10),data=panel,weights=weights)


out_reg_FS <- stargazer::stargazer(
  rFS, rFS_theta,
  header = FALSE, type = "latex", digits = 3, font.size = "tiny",
  dep.var.labels.include = FALSE,
  keep = c("time","tratamiento_control:time"),
  model.numbers = TRUE,
  label = "reg_FS", title = "FS",
  out = "results/tables/regs/reg_paper2_FS.tex",
  column.labels = c("FS","FS_Theta"),
  covariate.labels = c("Post","Tratamiento × Post"),
  table.placement = "H", column.sep.width = "-7pt", df = FALSE,
  omit.stat = c("f","adj.rsq"),                # << drop F and adj R2
  add.lines = list(
    c("Household FE", "Yes", "Yes"),
    c("Observations", format(nobs(rFS), big.mark=","), format(nobs(rFS_theta), big.mark=","))
  )
)
# ====================================
