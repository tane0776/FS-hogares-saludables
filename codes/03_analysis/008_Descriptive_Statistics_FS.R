...
vars <- read_excel("data/datasets/Variables_FS.xlsx")
...
# Food security → entra SIEMPRE (sin filtro de paper3_control / paper3_dep)
vars_foodsecurity <- vars$var_name[vars$Dimension == "Food security"]
...
# Harmonize labels and treatment column
ds$city <- dplyr::recode(ds$city, "Medellín" = "Medellin")
ds <- ds %>%
  dplyr::select(-dplyr::any_of(c("tratamiento_control.x",
                                 "tratamiento_control.y",
                                 "tratamiento_control_endline")))
# Ensure Food Security variables appear even if not flagged in the dictionary
vars_foodsecurity <- c(vars_foodsecurity, intersect(c("FSI","FSI_endline"), names(ds)))
...
get_desc_tx(ot,vars_foodsecurity,"FoodSecurity","ALL")
...
get_desc_tx(ot_med,vars_foodsecurity,"FoodSecurity","MED")
...
get_desc_tx(ot_cal,vars_foodsecurity,"FoodSecurity","CAL")
...
get_desc_tx(ot_bar,vars_foodsecurity,"FoodSecurity","BAR")
