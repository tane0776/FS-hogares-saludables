...
source("codes/02_processing/003b_Vars_laborales_endline.R") ## Output: LABORALES
...
.data_backup <- data        # guardo la data actual (individual)
data <- HOGARES             # paso la base de HOGAR al script del índice
source("codes/02_processing/003l_Vars_foodsecurity_indices.R")  # -> FOODSEC_INDEX (FSI, FSI_theta, FSI_theta_se) a nivel hogar
data <- .data_backup        # restauro la data original
rm(.data_backup)
# =============================================================================================

table(FOODSEC_INDEX$tratamiento_control)
length(FOODSEC_INDEX$tratamiento_control)
source("codes/02_processing/003k_Vars_endline.R") ## Output: ENDLINE
...
hs_main <- merge(hs_main,TIEMPO)
length(hs_main$tratamiento_control)

hs_main <- merge(hs_main, FOODSEC_INDEX, by = c("hh_id","tratamiento_control"), all.x = TRUE)
length(hs_main$tratamiento_control)

hs_main <- merge(hs_main, ENDLINE, by = c("hh_id","tratamiento_control"), all.x = TRUE)
length(hs_main$tratamiento_control)
...
###### Save - DS procesadas --> 
write_rds(hs_main,"data/survey/03_procesados/hogares_encuesta_procesada_endline_FS.rds")
...
write.csv(hs_main, file="data/survey/03_procesados/hogares_encuesta_procesada_endline_FS.csv", na="")
...
###### Save - DS procesadas --> 
write_rds(hs_main,"data/survey/03_procesados/hogares_encuesta_procesada_individual_endline_FS.rds")
...
write.csv(hs_main, file="data/survey/03_procesados/hogares_encuesta_procesada_individual_endline_FS.csv", na="")