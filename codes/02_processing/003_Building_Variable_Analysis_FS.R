...
### Build set of variables (igual que antes)
source("codes/02_processing/003a_Vars_demograficas.R") ## Output: DEMOGRAFICAS
table(DEMOGRAFICAS$tratamiento_control)
length(DEMOGRAFICAS$tratamiento_control)
source("codes/02_processing/003b_Vars_laborales.R") ## Output: LABORALES
table(LABORALES$tratamiento_control)
length(LABORALES$tratamiento_control)
source("codes/02_processing/003d_Vars_vivienda.R") ## Output: VIVIENDA 
table(VIVIENDA$tratamiento_control)
length(VIVIENDA$tratamiento_control)
source("codes/02_processing/003e_Vars_mentalhealth.R") ## Output: MENTAL
table(MENTAL$tratamiento_control)
length(MENTAL$tratamiento_control)
source("codes/02_processing/003f_Vars_Income.R") ## Output: INGRESOS
table(INGRESOS$tratamiento_control)
length(INGRESOS$tratamiento_control)
source("codes/02_processing/003g_Vars_saludfisica.R") ## Output: FISICO
table(FISICO$tratamiento_control)
length(FISICO$tratamiento_control)
source("codes/02_processing/003h_Vars_percepcionseguridad.R") ## Output: PERCEPCION
table(PERCEPCION$tratamiento_control)
length(PERCEPCION$tratamiento_control)
source("codes/02_processing/003i_Vars_ambientefamiliar.R") ## Output: PERCEPCION
table(PERCEPCION$tratamiento_control)
length(PERCEPCION$tratamiento_control)
source("codes/02_processing/003j_Vars_usotiempo.R") ## Output: TIEMPO
table(TIEMPO$tratamiento_control)
length(TIEMPO$tratamiento_control)

# ====== SOLO ESTE BLOQUE CAMBIA (mínimo): construir FSI a nivel HOGAR con el script PCM ======
# Guardar 'data' actual, cambiar temporalmente a HOGARES, sourcear, y restaurar
.data_backup <- data
data <- HOGARES
source("codes/02_processing/003l_Vars_foodsecurity_indices.R")  # -> FOODSEC_INDEX (FSI, FSI_theta, FSI_theta_se) a nivel hogar
data <- .data_backup
rm(.data_backup)
# =============================================================================================

table(FOODSEC_INDEX$tratamiento_control)
length(FOODSEC_INDEX$tratamiento_control)

######## Armamos el DS - A Nivel de hogar (igual que antes)
...
# Adjuntar el índice PCM a nivel hogar (mismo join key que ya usabas)
hs_main <- merge(hs_main, FOODSEC_INDEX, by = c("hh_id","tratamiento_control"), all.x = TRUE)
length(FOODSEC_INDEX$tratamiento_control)
...
#source("codes/02_processing/src_aux/Aux_003a_Price_Imputation.R")
#source("codes/02_processing/src_aux/Aux_003b_Area_Imputation.R")

###### Save - DS procesadas --> 
write_rds(hs_main,"data/survey/03_procesados/hogares_encuesta_procesada_FS.rds")
...
write.csv(hs_main, file="data/survey/03_procesados/hogares_encuesta_procesada_FS.csv", na="")

######## Armamos el DS - A Nivel de individuo (lo dejo intacto)
...
###### Save - DS procesadas --> 
write_rds(hs_main,"data/survey/03_procesados/hogares_encuesta_procesada_individual_FS.rds")
...
write.csv(hs_main, file="data/survey/03_procesados/hogares_encuesta_procesada_individual_FS.csv", na="")
